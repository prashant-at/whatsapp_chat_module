...
export class SocketService {
    ...
    setAuthCredentials(apiKey, phoneNumber, origin = '127.0.0.1') {
        ...
        console.log("[WA][Socket]   - API Key:", apiKey ? apiKey.substring(0, 20) + '...' : 'EMPTY');
        console.log("[WA][Socket]   - Phone Number:", phoneNumber || 'EMPTY');
        console.log("[WA][Socket]   - Origin:", origin);
    ...
    async disconnect() {
        if (this.socket) {
            ...
            this.socket = null;
            this.isConnected = false;
            console.log("[WA][Socket] ✅ Socket disconnected and cleaned up");
        }
    }
    ...
    async connect(userId, options = {}) {
        try {
            ...
            console.log("[WA][Socket] Client origin:", clientOrigin);
            console.log("[WA][Socket] API Key:", apiKey ? apiKey.substring(0, 20) + '...' : 'EMPTY');
            console.log("[WA][Socket] Phone Number:", phoneNumber || 'EMPTY');
            ...
            if (apiKey && phoneNumber) {
                ...
                console.log("[WA][Socket] ⚠️ Connecting without credentials - connection may fail");
            }

            console.log("[WA][Socket] Connection options:", socketOptions);
            ...
            console.error("   - Backend doesn't have Socket.IO server");
            console.error("   - CORS not configured for localhost:8069");
    ...
    setupEventHandlers() {
        ...
        this.socket.on('connect', () => {
            ...
            console.log("🔗 [Socket Event] CONNECT - Socket ID type:", typeof this.socket.id);
        ...
        this.socket.on('disconnect', (reason) => {
            ...
            console.log("🔌 [Socket Event] DISCONNECT - Reason type:", typeof reason);
            console.log("🔌 [Socket Event] DISCONNECT - Is connected was:", this.isConnected);
            this.isConnected = false;
            
            // Check if server disconnected due to authentication failure
            if (reason === 'io server disconnect') {
                console.error("⚠️ Server forcefully disconnected. Possible reasons:");
                console.error("   - Invalid API credentials");
                console.error("   - Phone number doesn't match registered");
                console.error("   - Origin doesn't match registered IP");
        ...
        this.socket.on('connect_error', (error) => {
            console.error("❌ [Socket Event] CONNECT_ERROR - Socket.IO connection error:", error);
            console.error("❌ [Socket Event] CONNECT_ERROR - Error type:", typeof error);
        ...
        this.socket.on('qr_code', ({ data }) => {
            console.log("📱 [Socket Event] QR_CODE received:", data);
        ...
        this.socket.on('phone_mismatch', ({ data }) => {
            console.log("📱 [Socket Event] PHONE_MISMATCH received:", data);
        ...
        this.socket.on('status', ({ data }) => {
            console.log("📱 [Socket Event] STATUS received:", data);
        ...
        this.socket.on('chat', ({ data }) => {
            console.log("📱 [Socket Event] CHAT received:", data);
    ...
    updateQrPopupUI(data, retryCount = 0) {
        console.log(`[QR Popup] 🔄 Updating UI with data (attempt ${retryCount + 1}):`, data);
        let qrCode = null;
        ...
        if (qrImg && qrCode) {
            ...
            if (retryCount < maxRetries) {
                console.log(`[QR Popup] ⚠️ QR image element not found (attempt ${retryCount + 1}/${maxRetries}), retrying in ${retryDelay}ms...`);
                setTimeout(() => {
                    this.updateQrPopupUI(data, retryCount + 1);
                }, retryDelay);
                return; // Exit early, will retry
            } else {
                console.warn('[QR Popup] ⚠️ QR image element not found after max retries - popup may not be open');
            }
        } else if (qrImg && !qrCode) {
            console.warn('[QR Popup] ⚠️ QR image found but no qrCode in data');
        }
        ...
        // 3️⃣ (Optional) Reset countdown timer if provided
        const countdown = document.querySelector('.qr-countdown span') ||
                         document.querySelector('[name="countdown_display"]');
        if (countdown && data.countdown !== undefined) {
            countdown.textContent = `${data.countdown}s`;
            console.log('[QR Popup] ⏱ Countdown updated');
        }

        // 4️⃣ (Optional) Update live counter
        const counter = document.querySelector('.o_field_widget[name="qr_update_count"]') ||
                       document.querySelector('field[name="qr_update_count"]');
        if (counter) {
            const current = parseInt(counter.textContent || "0");
            counter.textContent = current + 1;
            console.log('[QR Popup] 🔁 Update counter incremented');
        }
    ...
    sendRPC(type, data) {
        ...
        .then((payload) => {
            ...
            if (!result) {
                console.warn('[Socket Event] ⚠️ RPC returned no result:', payload);
                return;
            }

            console.log(`[Socket Event] ✅ RPC call successful for ${type}:`, result);
        ...
        .catch(error => {
            console.error('[Socket Event] ❌ RPC fetch error:', error);
