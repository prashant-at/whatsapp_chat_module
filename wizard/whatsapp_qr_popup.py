...
class WhatsAppQRPopup(models.TransientModel):
    ...
    def update_qr_from_socket(self, qr_data):
        ...
        try:
            ...
            if not popup:
                _logger.warning(f"⚠️ [QR Popup] No popup found for socket update")
                return False
            
            # Extract QR data
            qr_img = qr_data.get('qrCode') or qr_data.get('qr_img') or qr_data.get('qr_code_image')
            if not qr_img:
                _logger.warning(f"⚠️ [QR Popup] No QR image data in socket event")
                return False
    ...
    def handle_socket_qr_update(self, qr_data):
        ...
        try:
            ...
            if not qr_code:
                _logger.warning(f"⚠️ [QR Popup] No QR code data in socket event")
                return False
    ...
    def _handle_phone_mismatch_event(self, mismatch_data):
        ...
        try:
            ...
            if not popup:
                _logger.warning(f"⚠️ [QR Popup] No popup found for phone mismatch event")
                return False
            ...
            if qr_code:
                ...
                _logger.warning(f"⚠️ [QR Popup] No QR code in phone mismatch event")
                return False
    ...
    def _handle_status_event(self, status_data):
        ...
        try:
            ...
            #     return True
                ...
                _logger.warning(f"⚠️ [QR Popup] Phone mismatch detected")
                ...
                _logger.warning(f"⚠️ [QR Popup] Disconnected")
        ...
        try:
            ...
            if isinstance(data, dict):
                ...
                if 'qrCode' in data:
                    ...
                    # _logger.warning(f"⚠️ [QR Popup] No QR code found in data structure: {data}")
                    return False
                ...
                _logger.warning(f"⚠️ [QR Popup] Unexpected data format: {data}")
                return False
            
            if not qr_code:
                _logger.warning(f"⚠️ [QR Popup] QR code is empty")
                return False
